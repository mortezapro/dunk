...
@section("breadcrumb")
    <div class="row heading-bg">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h5 class="txt-dark">درخواست پول</h5>
        </div>
        <!-- Breadcrumb -->
        ...
        <table id="tbl_question" class="table table-hover">
            ...
            @foreach($requests as $request)
                ...
                <tr class="@if($request->request_state==0)bg-read @else bg-no-read @endif">
                    <td>{{$i}}</td>
                    <td>{{$request->users->user_flname}}</td>
                    <td>{{$request->users->user_mobile}}</td>
                    <td>{{$request->request_coin}}</td>
                    <td>{{\Morilog\Jalali\Jalalian::forge($request->request_date)->format('%d %B %Y')}}</td>
                    <td>
                        <a href="{{route("adminRequestMoneyDetailsRoute",["id"=>$request->request_id])}}" class="btn btn-primary">مشاهده</a>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer2">
                            <a href="{{route("adminRequestMoneyDeleteRoute",["id"=>$request->request_id])}}"><button class="btn btn-danger"><i class="fa fa-edit"></i> بله</button></a>
