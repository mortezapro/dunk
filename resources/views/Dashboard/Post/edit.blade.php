                ...
                <div class="panel-wrapper collapse in">
                    <form action="{{route("updatePost")}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-9">
                                <div class="panel">
                                    <input style="display: none" value="{{$post->id}}" type="text" id="post_id" name="post_id"  class="form-control">

                                    <div class="panel-body">
                                        <input class="form-control @error('title') is-invalid @enderror" type="text" value="{{$post->title}}" onkeyup = changeSlug(event) name="title" id="title" placeholder="عنوان پست"/>
                                        @error('title')
                                        ...
                                        @enderror
                                    <div class="panel-body">
                                        <input class="form-control @error('slug') is-invalid @enderror" type="text" value="{{$post->slug}}" name="slug" id="slug" placeholder="آدرس پست"/>
                                        @error('slug')
                                        <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                    ...
                                    <div class="panel-body">
                                        <div class="category-post">
                                            ...
                                            <select class="custom-select custom-select-sm @error('status') is-invalid @enderror" name="status">
                                                <option @if ($post->status==2024 )selected @endif  value="2024">انتشار</option>
                                                <option @if ($post->status==2023 )selected @endif value="2023">پیشنویس</option>
                                            </select>
                                            @error('status')
                                            <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    ...
                                    <div class="panel-body">
                                        <div class="category-post">
                                            ...
                                            <select class="custom-select custom-select-sm @error('category') is-invalid @enderror" name="category">
                                                <option  disabled hidden value="">انتخاب کنید</option>
                                                @foreach($category as $item)
                                                    <option @if($post->category_id==$item->category_id)selected @endif value="{{$item->category_id}}">{{$item->category_name}}</option>
                                                @endforeach
                                            </select>
                                            @error('category')
                                            <span class="invalid-feedback" role="alert">
